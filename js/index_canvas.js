...
// }
  //页面操作js
      ...
      startGame = function(){
          ...
          planeMoveTimer = setInterval(PlaneMove,50);//小飞机移动
          smallPlaneTimer = setInterval(createSmallPlane,1500);//创建小飞机
          setTimeout(function(){console.log('开始创建大飞机!');bigPlaneTimer = setInterval(createBigPlane,3000);},3000);//创建大飞机
          setTimeout(function(){console.log('开始创建加速道具!');speedTimer = setInterval(createDouble,5000);},3000);//创建double子弹
          // setTimeout(function(){ console.log('没有无敌么？');document.getElementById('player').src="./img/本机_无敌.png";},2000);
          setTimeout(function(){console.log('开始创建无敌道具!');protectTimer = setInterval(createProtect,5000);},4000);//创建无敌
          setTimeout(function(){console.log('开始创建frog道具!');protectTimer = setInterval(createFrog,5000);},5000);//创建frog
      ...
      continuegame = function(){
          ...
          planeMoveTimer = setInterval(PlaneMove,50);//小飞机移动
          smallPlaneTimer = setInterval(createSmallPlane,1500);//创建小飞机
          setTimeout(function(){console.log('开始创建大飞机!');bigPlaneTimer = setInterval(createBigPlane,3000);},1000);//创建大飞机
          setTimeout(function(){console.log('开始创建加速道具!');speedTimer = setInterval(createDouble,5000);},3000);//创建double子弹
          setTimeout(function(){console.log('开始创建无敌道具!');protectTimer = setInterval(createProtect,5000);},4000);//创建无敌
          ctrlPlayTimer = setInterval(ctrlPlay,30);//30毫秒监听一次 监听键盘按键
          bulletMoveTimer = setInterval(bulletMove,10);//子弹的移动
  ...
  //游戏内的js    
      ...
      */
      function playerPlaneProto(imgSrc,x,y,speed){
          ...
          img.onload=function () {
              cansText.drawImage(img,x,y);   //在画布X轴 50  Y轴坐标50 处添加一张图片
          };
          // this.speed=speed;
          this.playerIsDead = false;
          this.init = function(){
              mainObj.appendChild(canvas);
          }
          this.init();
...
//无敌道具
      ...
      function userCrash(){
          for(var i=0;i<smallPlaneArray.length;i++){
              ...
              if (smallPlaneArray[i].isDead == false) {
                  if (plLeft2>=playerLeft&&plLeft2<=playerLeft+100&&plTop2>=playerTop&&plTop2<=playerTop+100) {
                      if(!protect_flag)
